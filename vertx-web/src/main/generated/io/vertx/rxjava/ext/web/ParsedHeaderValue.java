...
public class ParsedHeaderValue {
  ...
  public ParsedHeaderValue(io.vertx.ext.web.ParsedHeaderValue delegate) {
  ...
  public ParsedHeaderValue findMatchedBy(Set<ParsedHeaderValue> matchTries) { 
    ParsedHeaderValue ret = ParsedHeaderValue.newInstance(delegate.findMatchedBy(matchTries.stream().map(elt -> (io.vertx.ext.web.ParsedHeaderValue)elt.getDelegate()).collect(java.util.stream.Collectors.toSet())));
    return ret;
  }
  ...
  public static ParsedHeaderValue newInstance(io.vertx.ext.web.ParsedHeaderValue arg) {
    return arg != null ? new ParsedHeaderValue(arg) : null;
  }
}
